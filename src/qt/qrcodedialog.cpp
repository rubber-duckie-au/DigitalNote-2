...
{
    QString ret = QString("DigitalNote:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non XDN unit input in lnReqAmount, we generate the URI with XDN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(DigitalNoteUnits::format(DigitalNoteUnits::XDN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
